...
Mathematically, maximal inequalities are concerned with suprema of a stochastic process $(X_\theta)_{\theta\in \Theta}$ where $\Theta$ is an index set in some space—often in $\Re^d$ but more generally a [[Hilbert space]] or a [[Banach space]].  That is, we usually want to bound
$$
